...
public class StoreStepsDataWorker extends Worker{
    private static final String TAG = StoreStepsDataWorker.class.getSimpleName();
    ...
    public Result doWork() {
        ...
        try {
            new Thread(()->{
                ...
                List<FitDataModel> dataModels = dataService.getDataListOfType(AppConst.STEPS);

                if(dataModels != null && !dataModels.isEmpty()){
                    NetworkApi.on().storeStepsOrDistanceData(AppConst.STEPS, dataModels, new StoreSensorDataCallback() {
                        @Override
                        public void onStoreSensorDataSuccessfully(@NotNull String dataId) {
                            Log.e(TAG, "ReMap Steps Data stored: " + dataId);
                        }
                    ...
                    dataService.deleteDataOfType(AppConst.STEPS);

                    FileUtil.addNewLogOnSD("Total step data sent " + dataModels.size() + " items");
                }
            ...
            Log.e(TAG, "Error storing step data", throwable);
